...
export function CodeWrapper(props): JSX.Element {
...
export function SpanWrapper(props): JSX.Element {
  // eslint-disable-next-line prefer-const
  let { children, class: className, ...rest } = props;
  if (children) {
    children = makeKeywordsClickable(children);
...
const CodeBlock = ({ filename, language, children }: Props): JSX.Element => {
  ...
  return (
    <div className="code-block">
      ...
      <div ref={codeRef}>
        <HighlightCode language={language}>{children}</HighlightCode>
      </div>
    </div>
  );
};
